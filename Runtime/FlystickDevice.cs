...
public class FlystickDevice : InputDevice, IInputUpdateCallbackReceiver
{
    ...
    VRPN_Analog _analog = Transform.FindObjectOfType<VRPN_Analog>();
    ...
    public void OnUpdate()
    {
        if (!listenersAdded)
        {
            ...
            //_analog.OnAnalog.AddListener(SetAnalog);
        ...
        if (_analog.Analog.x > 0)
        {
            Debug.Log("Reading Analog (" + _analog.Analog.x + ", " + _analog.Analog.y + ")");
            state.x += 127;
        }
        else if (_analog.Analog.x < 0)
        {
            state.x -= 127;
        }
        if (_analog.Analog.y > 0)
        {
            state.y += 127;
        }
        else if (_analog.Analog.y < 0)
        {
            state.y -= 127;
        }
    ...
    private void SetAnalog(Vector2 arg0, FlystickDeviceState state)
    {
        Debug.Log("Receiving Analog (" + arg0.x + ", " + arg0.y + ")");
        if (arg0.x > 0)
        {
            state.x += 127;
        }
